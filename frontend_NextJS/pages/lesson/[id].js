...
const fetchData = async (id, onClient = false) => {
  const backendUrl = onClient
    ? process.env.NEXT_PUBLIC_BACKEND_PORT_LOCAL
    : process.env.NEXT_PUBLIC_BACKEND_PORT;

  const getCategoryUrl = `${backendUrl}/lesson/categories?id=${id}`;
  const getLessonsUrl = `${backendUrl}/lesson?category=${id}`;
...
const CertainLessonPage = ({ categoryId, category, initLessons }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { lessons } = await fetchData(categoryId, true);
      setLessons(lessons);
    })();
  }, [auth]);
