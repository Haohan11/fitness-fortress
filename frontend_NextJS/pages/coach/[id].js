...
const fetchData = async (id, onClient = false) => {

  const baseUrl = onClient ? process.env.NEXT_PUBLIC_BACKEND_PORT_LOCAL : process.env.NEXT_PUBLIC_BACKEND_PORT
  const getCoachUrl = `${baseUrl}/coach/${id}`;
  const getLessonsUrl = `${baseUrl}/lesson?coach=${id}`;
...
const CoachPage = ({ coach, coachId, initLessons }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { lessons } = await fetchData(coachId, true);
      setLessons(lessons);
    })();
  }, [auth]);
